...
- **Description:** Included in this directory are raw representative photographs (as .jpg files; file naming convention: "GOPROXXX.JPG") of photoquadrats used for creating "metacommunity_BCM_benthic_df.csv" and raw representative videos (as .mp4 files; file naming convention: "SITE_DATE_MATID.mp4") analyzed to create "mat_video_follow_counts.csv". Note that video quality was reduced using HandBrake v 1.10.2 to allow for upload. Also note that the included video files do not encompass the complete duration of each follow, as complete follow durations were often split across several unique video files. Full datasets are available upon request (contact ethan.c.cissell@gmail.com)
